...
- (CGSize)calculateContentSizeThatFits:(CGSize)size andUpdateLayout:(BOOL)update
{
    ...
    if (self.theme.popupStyle ==  CNPPopupStyleCentered) {
        UIButton *closerButton = [UIButton buttonWithType:UIButtonTypeCustom];
        [closerButton setImage:[UIImage imageNamed:@"close"] forState:UIControlStateNormal];
        closerButton.frame = CGRectMake(result.width -10 , 0, 44, 44);
        [closerButton addTarget:self
                         action:@selector(closeButtonPressed:) forControlEvents:UIControlEventTouchUpInside];
        [self.popupView addSubview:closerButton];
    }
...
- (void)closeButtonPressed:(id)sender {
    [self dismissPopupControllerAnimated:true];
}
